...
int main()
{
	...
	{
		dateStoc1.seekg(0, ios::end);
		int nrElem = dateStoc1.tellg() / sizeof(Wrapper);
		cout << "numarul de elemente din fisier: " << dateStoc1.tellg() / sizeof(Wrapper) << endl;
		...
		while (!dateStoc1.eof())
		{
			...
			dateStoc1 >> aux;
		//	cout << aux;
		//	vect.push_back(Wrapper(aux.produs.idProdus, aux.produs.numeProdus, aux.produs.pretProdus, aux.cantitateProdus));
			//mapul.insert({ aux.produs.idProdus, aux });
			magazin.adaugaProdus(Wrapper(aux.produs.idProdus, aux.produs.numeProdus, aux.produs.pretProdus, aux.cantitateProdus));
			//	cout << aux;
				//magazin.adaugaProdus(aux);

				/*magazin.adaugaProdus(aux);
				magazin.afiseazaProduse();*/

				/*	dateStoc1 >> vectorWrapper1;
					cout << vectorWrapper1;*/
	...
	//for (int i = 0; i < vectorWrapper.size(); i++)
	//{
	//	cout << vect[i];
	//}
	//map<int, Wrapper>::iterator itStoc;
//for (itStoc = mapul.begin(); itStoc != mapul.end(); itStoc++)
//{
//	cout
//		//	<< itStoc->first << "\t" 
//		<< itStoc->second;
